...
"""
function plot_place_field!(lg, celldir::String;nrefinements=(p=3,g=2),σ=3, α=1000.0.^2, kwargs...)
    ...
    viz!(ax1, mm;showsegments=true, color=Z)
    Colorbar(lg[1,2], colorrange=(extrema(filter(isfinite, Z))))
    ax1.title = "Raw"
    Zg = smg.weight./smg.occupancy
    Zg[smg.unvisited] .= NaN 
    viz!(ax2, mm;showsegments=true, color=Zg)
    Colorbar(lg[1,4], colorrange=(extrema(filter(isfinite, Zg))))

    ax2.title = "σ = $(σ)"
    Za = sma.weight./sma.occupancy
    Za[sma.unvisited] .= NaN
    viz!(ax3, mm;showsegments=true, color=Za)
    Colorbar(lg[1,6], colorrange=(extrema(filter(isfinite, Za))))
    ax3.title = "α = $α"
    ax1,ax2,ax3
end